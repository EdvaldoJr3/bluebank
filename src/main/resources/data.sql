...
BEGIN
	DECLARE qtd INT DEFAULT 0;
	 ...
	 -- ROLES
    SELECT COUNT(*) INTO qtd FROM role WHERE role_id = 1 and role = 'ADMIN';
    IF qtd = 0 THEN
	...
	-- TIPO OPERACAO
    SET qtd = 0;
    SELECT COUNT(*) INTO qtd FROM tipo_operacao;
    IF qtd = 0 THEN
        INSERT INTO `tipo_operacao` (`id`, `data_alteracao`, `descricao`) VALUES (1, now(), 'Crédito');
        INSERT INTO `tipo_operacao` (`id`, `data_alteracao`, `descricao`) VALUES (2, now(), 'Débito');
        COMMIT;
	END IF;

	-- AGENCIA
    SET qtd = 0;
    SELECT COUNT(*) INTO qtd FROM agencia;
    IF qtd = 0 THEN
        INSERT INTO `agencia` (`id`, `data_alteracao`, `digito`, `numero`, `status`)
    ...
    IF qtd = 0 THEN
        INSERT INTO `cliente` (`id`, `cpf`, `data_alteracao`, `email`, `endereco`, `nome`, `status`)
	...
	-- CONTA CORRENTE
    SET qtd = 0;
    SELECT COUNT(*) INTO qtd FROM conta_corrente;
    IF qtd = 0 THEN
        INSERT INTO `conta_corrente` (`id`, `agencia_id`, `cliente_id`, `data_alteracao`, `digito`, `numero`, `status`)
    ...
    IF qtd = 0 THEN
        INSERT INTO `usuario` (`id`, `cpf`, `data_alteracao`, `senha`, `status`, `cliente_id`)
	...
	-- USUARIO_ROLE
    SET qtd = 0;
    SELECT COUNT(*) INTO qtd FROM usuario_role;
    IF qtd = 0 THEN
        INSERT INTO `usuario_role` (`id`, `role_id`) VALUES ('1', '1');
        INSERT INTO `usuario_role` (`id`, `role_id`) VALUES ('2', '1');
        INSERT INTO `usuario_role` (`id`, `role_id`) VALUES ('3', '1');
        INSERT INTO `usuario_role` (`id`, `role_id`) VALUES ('4', '1');
        COMMIT;
	END IF;

	-- CONTA_CORRENTE_SALDO
    SET qtd = 0;
    SELECT COUNT(*) INTO qtd FROM conta_corrente_saldo;
    IF qtd = 0 THEN
        INSERT INTO `conta_corrente_saldo` (`id`, `conta_corrente_id`, `data_alteracao`, `saldo`, `versao`)
